...
- Every pipeline execution must persist a structured, non-secret run report and a human-readable run narrative, including success, failure, interruption, partial, resumed, and no-op outcomes. Preserve every rejected generated candidate byte-for-byte in the entity output area with collision-safe run/attempt naming and no appended framework content. Preserve its complete actionable rejection record in a same-basename `.explanation.md` sidecar bound to the candidate path and SHA-256. Reports must link both files plus every rejection/retry reason, authority, feedback, prompt growth, retry disposition, and final outcome, and may claim preservation only after the pair exists. Rejected candidates and sidecars are ignored, untrusted evidence: never promote, render as final output, retrieve as examples, treat as source facts, or automatically assemble them into prompts. Use `references/run_evidence_and_continuous_improvement.md` for the complete rationale and contract.
