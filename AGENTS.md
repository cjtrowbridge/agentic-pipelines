...
- Every pipeline execution must persist a structured, non-secret run report and a human-readable run narrative, including success, failure, interruption, partial, resumed, and no-op outcomes. Preserve every rejected generated candidate separately in the entity output area with collision-safe naming and an appended actionable rejection record. Reports must link attempts, rejected artifacts, every rejection/retry reason, authority, feedback, prompt growth, retry disposition, and final outcome. Rejected artifacts are ignored, untrusted evidence: never promote, render as final output, retrieve as examples, treat as source facts, or automatically assemble them into prompts. Use `references/run_evidence_and_continuous_improvement.md` for the complete rationale and contract.
