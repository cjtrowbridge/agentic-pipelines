...
| Audit an existing host pipeline against current governance | `playbooks/how_to_audit_existing_pipeline_conformance.md` |
...
- Deterministic methods may establish only properties exactly computable from declared representations or traceable exact domain rules. Meaning, entailment, equivalence, relevance, faithful restatement, and qualitative fitness require bounded semantic or explicit human judgment. Heuristics may route or escalate uncertainty but may not impersonate semantic proof. Use `references/deterministic_and_semantic_authority.md` when designing or auditing gates.
- Design model work around the smallest coherent semantic decision, not the smallest individually nameable claim. “Narrow” means bounded goal, inputs, outputs, authority, and failure behavior; do not fragment context or create semantic microstages merely to make a proxy easier to validate.
- LLMs are few-shot learners. When trusted representative examples exist, prefer bounded past input/accepted-output pairs followed by the new input over expanding handcrafted rules that approximate the demonstrated semantic behavior. Declare each example's authority, provenance, selection, whole-session budget, and omission evidence.
- Prohibit semantic laundering: deterministic code may validate a proxy's exact representation but may not let a heuristic, inferred free-text class, model label, typed evidence kind, similarity score, or lexical relation decide the meaning it approximates.
- Scope semantic review to the decision currently being made. A requested revision audits the requested delta and its direct consequences; supported operator intent is blocking authority within policy, while broader reviewer preference is advisory. Fresh-session independent review and claim-level evidence are risk-justified high-assurance options, not universal defaults.
...
- Every review of a task execution plan must review the governing roadmap at the same time. Compare their scope, status, sequencing, dependencies, deliverables, milestones, and acceptance conditions; report every mismatch and propose concrete changes to the plan, the roadmap, or both. Apply approved corrections in the same checkpoint so the plan and roadmap remain consistent. If either artifact is absent or cannot be changed under the current authority, record that fact and the unresolved mismatch explicitly; a plan review is not complete merely because the plan is internally coherent.
...
- Every model request must have a pre-transport packet manifest: allowed source roles, selected and omitted source IDs, component sizes, static prompt size, complete assembled-request size, context/completion limits, and any batching or reduction reason. Bounding one component is not sufficient. Unknown, generated, rejected, diagnostic, configuration, and otherwise forbidden source roles are excluded before prompt assembly. An optional stage must declare its fallback and cannot abort unrelated required work; a provider outage opens a run-local circuit, preserves evidence, leaves unattempted model work pending, and reaches a truthful terminal report without a traceback. Use `references/source_packets_and_provider_resilience.md`.
...
- Every pipeline execution must persist a structured, non-secret run report and a human-readable run narrative, and checkpoint both after material events, including success, failure, interruption, partial, resumed, and no-op outcomes. Preserve every rejected generated candidate byte-for-byte in an entity-local, naturally sortable evidence bundle named `<artifact>.<sequence>.rejected.<ext>` with a same-basename `.explanation.md` sidecar. Associated precursor evidence uses `<artifact>.<sequence>.rejected.<stage>.<ext>` and its own same-basename sidecar; run/session/attempt IDs and hashes belong in metadata, never the human filename. Extensions must state the actual representation, and downstream failure must preserve the parent candidate rather than strand it in a thread envelope. Every newly rejected renderable Markdown candidate must receive an isolated deterministic diagnostic render when its declared renderer is available: `<artifact>.<sequence>.rejected.render.pdf` plus its own sidecar. Diagnostic rendering reads the exact persisted bytes and is evidence-only; it is never final rendering, promotion, repair, a model call, or a retry loop. A renderer failure preserves the parent and records a typed `.render.txt` child. Reports must link the complete bundle plus every rejection/retry reason, authority, feedback, prompt growth, retry disposition, and final outcome, and may claim preservation only after the pair exists. Rejected evidence is ignored and untrusted: never promote, use in ordinary/final rendering, retrieve as examples, treat as source facts, or automatically assemble it into prompts. Use `references/run_evidence_and_continuous_improvement.md` for the complete rationale and contract.
