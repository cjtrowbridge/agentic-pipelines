...
- [ ] 2. Verify and publish the framework change.
  ...
  - [ ] 2.3 Commit and push the framework revision to `origin/main`.
- [ ] 3. Propagate the published revision.
  - [ ] 3.1 Update the parent repository's direct `agentic-pipelines` gitlink.
  - [ ] 3.2 Update and publish the `iftf-thor-runtime` nested gitlink.
  - [ ] 3.3 Initialize/update and publish the `iftf_ebe_runtime` nested gitlink without including unrelated worktree changes.
  - [ ] 3.4 Record the two updated runtime revisions in the parent repository and push the parent gitlinks without including unrelated plan changes.
